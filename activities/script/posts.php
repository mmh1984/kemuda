...
echo "</table>";
	...
	echo "<p class='alert alert-warning' align='center'><strong>No upcoming events</strong></p>";	
}
mysqli_close($con);
...
tblposts.id DESC LIMIT 1");
  echo "<h4><span class='glyphicon glyphicon-gift'></span> Latest Post</h4>";
	while($row=mysqli_fetch_array($query)){
		echo "<img src='imagepost/$row[3]' class='img img-thumbnail img-responsive center-block'>";
		echo "<h2><a href='viewpost.php?id=$row[0]'>$row[1]</a></h2>";
